...
const Accordion = ({ title, id, desc, selected, onClick }) => {
  ...
  return (
    <div
      className={`shadow-accordion accordion ${
    ...
    >
      ...
      >
        ...
        {/* accordion icon */}
        <img
          className={`w-[16px] h-[16px] object-contain transition-main ${
            isOpen ? "rotate-180" : ""
          }`}
          src={iconArrowBottom}
          alt="icon arrow"
      ...
      <div
        ...
        style={
          isOpen
            ? {
                height: elContent?.current?.scrollHeight + 45 + "px" || "110px",
              }
            : {}
        }
      ...
      >
        <p className="font-17 font-normal text-[#fff]/[.9] leading-[1.4]">
          {desc}
        </p>
      </div>
    </div>
